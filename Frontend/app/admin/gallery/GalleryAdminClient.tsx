  ...
  return (
    <div className="space-y-8 max-w-6xl">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-aura-gold/20 pb-4">
        <div>
          <h1 className="font-serif text-3xl font-bold text-aura-ink">Lookbook & Editorial Gallery</h1>
          <p className="text-xs text-aura-subink mt-1">Manage photoshoot highlights and Instagram editorial photos.</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
        {loading ? (
          <p className="text-xs text-aura-subink">Loading photos...</p>
        ) : items.map((item) => (
          <div key={item.id} className="relative aspect-[3/4] rounded-xl overflow-hidden border border-aura-gold/20 group">
            <Image src={item.storage_path} alt={item.caption || 'AURA'} fill className="object-cover" />
            <div className="absolute inset-0 bg-aura-bg/80 opacity-0 group-hover:opacity-100 transition-opacity p-4 flex flex-col justify-between">
              <p className="text-xs font-serif text-aura-ink font-bold">{item.caption}</p>
              <button
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 bg-aura-bg/90 backdrop-blur-md flex items-start justify-center p-4 pt-10 pb-10 overflow-y-auto" data-lenis-prevent="true">
          <div className="w-full max-w-md bg-aura-surface border border-aura-gold/30 rounded-2xl p-6 space-y-6 relative shadow-2xl my-auto shrink-0">
            ...
            <h2 className="font-serif text-2xl font-bold text-aura-ink">Add Lookbook Item</h2>

            <form onSubmit={handleSave} className="space-y-4 text-xs">
              <div className="space-y-1">
                <label className="font-semibold text-aura-ink">Caption</label>
                <input
                  type="text"
              ...
              <div className="space-y-1">
                <label className="font-semibold text-aura-ink">Instagram Link (Optional)</label>
                <input
                  type="url"
              ...
              <div className="space-y-2">
                <label className="font-semibold text-aura-ink block">Image URL or File Upload *</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full bg-aura-elevated border border-aura-gold/20 text-aura-ink p-3 rounded"
                />
                <div className="relative border border-dashed border-aura-gold/30 p-4 rounded text-center bg-aura-elevated/50">
                  <input type="file" accept="image/*" onChange={handleFileUpload} className="absolute inset-0 opacity-0 cursor-pointer" />
                  <Upload className="w-5 h-5 text-aura-gold mx-auto mb-1" />
