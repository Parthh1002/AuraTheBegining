...
export default function SettingsAdminClient() {
  ...
  if (loading) return <div className="p-8 text-xs text-[#9C9894]">Loading settings...</div>;

  return (
    <div className="space-y-8 max-w-4xl">
      <div className="flex items-center justify-between border-b border-[#D4A02A]/20 pb-4">
        <div>
          <h1 className="font-serif text-3xl font-bold text-[#F5F1E8]">Store Settings</h1>
          <p className="text-xs text-[#9C9894] mt-1">Configure contact numbers, Dahegam address, store hours, and hero banner.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSaveSettings} className="space-y-6 text-xs">
        <div className="bg-[#151517] border border-[#D4A02A]/20 rounded-xl p-6 space-y-4 shadow-xl">
          <h3 className="font-serif text-lg font-bold text-[#F5F1E8] border-b border-[#D4A02A]/10 pb-2">
            Store Contact & Location
          </h3>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="font-semibold text-[#F5F1E8]">Store Phone</label>
              <input
                type="text"
            ...
            <div className="space-y-1">
              <label className="font-semibold text-[#F5F1E8]">WhatsApp Number (digits only)</label>
              <input
                type="text"
          ...
          <div className="space-y-1">
            <label className="font-semibold text-[#F5F1E8]">Store Address</label>
            <textarea
              rows={2}
        ...
        <div className="bg-[#151517] border border-[#D4A02A]/20 rounded-xl p-6 space-y-4 shadow-xl">
          ...
          <div className="space-y-1">
            <label className="font-semibold text-[#F5F1E8]">Hero Main Headline</label>
            <input
              type="text"
              value={formData.hero_headline}
              onChange={(e) => setFormData((prev) => ({ ...prev, hero_headline: e.target.value }))}
              className="w-full bg-[#1D1D20] border border-[#D4A02A]/20 text-[#F5F1E8] p-3 rounded font-serif text-base"
            />
          </div>

          <div className="space-y-1">
            <label className="font-semibold text-[#F5F1E8]">Hero Subtext</label>
            <textarea
              rows={2}
          ...
          <div className="space-y-2">
            <label className="font-semibold text-[#F5F1E8] block">Hero Media Image URL / File Upload</label>
            <input
              type="text"
        ...
        <button
          type="submit"
          disabled={saving}
          className="bg-[#D4A02A] text-[#0A0A0C] font-bold text-xs uppercase tracking-widest px-8 py-4 rounded hover:bg-[#E8C168] transition-colors flex items-center justify-center gap-2 cursor-pointer shadow-lg disabled:opacity-50"
        >
          {saving ? (
            <>
